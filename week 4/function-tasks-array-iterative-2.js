...
console.groupCollapsed('1. Atspausdinti visus Informatikos fakulteto studentus');
{
    students.forEach((student) => {
        if (student.faculty === 'Informatikos fakultetas') console.log(student);
    });
}
console.groupEnd();
...
{
    students.forEach((student) => {
        if (student.faculty === 'Elektros ir elektronikos fakultetas') console.log(student);
    });
}
console.groupEnd();
    ...
    students.forEach((student) => {
        if (student.faculty === 'Elektros ir elektronikos fakultetas') {
            ...
            student.modules.forEach((module) => {
                const partialMarks = module.marks.length;
                const partialSum = module.marks.reduce(
                    (accumulator, currentValue) => accumulator + currentValue
                );
                sum += partialSum;
                marks += partialMarks;
            });
            console.log(`${student.name} ${student.surname} => ${sum / marks}`);
        }
    });
}
console.groupEnd();
